...
def render_item(caption, checked):
    ...
    # Generate completely unique ID for checkbox
    import time
    import random
    import uuid
    unique_id = f"checkbox_{int(time.time() * 1000000)}_{random.randint(100000, 999999)}_{uuid.uuid4().hex[:8]}"

    return f"<li class=\"option-item\">" \
           f"<input type=\"checkbox\" data-question=\"{fake}\" data-content=\"{correct}\" id=\"{unique_id}\" />" \
